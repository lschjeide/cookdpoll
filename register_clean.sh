...
OLD_INSTANCE_ID=$(aws ec2 describe-instances   --region=ap-southeast-2   --filter "Name=tag:Live,Values=false"   --query='Reservations[*].Instances[*].InstanceId'   --output=text)
